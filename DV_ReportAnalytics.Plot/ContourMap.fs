...
module public ContourMap =
    let CreateInlineHTML(table : TableInfo, layout : Layout, colorscale : obj list list) : string =
        let chart =
            Contour(x = table.ColumnHeader, y = table.RowHeader, z = table.DataBody, colorscale = colorscale)
            |> Chart.Plot
            |> Chart.WithLayout layout
            |> Chart.WithXTitle("Speed")
            |> Chart.WithYTitle("Torque")
            |> Chart.WithWidth 700
